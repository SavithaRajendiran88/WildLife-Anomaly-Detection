...
def build_window_from_points(points: list[dict], scaler) -> np.ndarray:
    """
    points: list of 24 dicts with keys lat, lon, speed_kmh, distance_km
    Returns shape (1, 24, 4) scaled array.
    """
...
def predict(window: np.ndarray, elephant_id: str):
...
with st.sidebar:
    ...
    <div style='padding:1rem 0'>
      <div style='font-family:Space Mono,monospace;font-size:0.6rem;letter-spacing:3px;color:#6B8F74;text-transform:uppercase;margin-bottom:0.5rem'>System Status</div>
    </div>
    """, unsafe_allow_html=True)
    ...
    if scaler is not None:
        ...
        st.warning("⚠ scaler.pkl not found — raw values used (accuracy may vary)")
    ...
    Best model from a suite of 9 architectures (MLP, CNN, RNN, LSTM, GRU, Attention-BiLSTM, ResNet, MobileNet, Autoencoder, GAN).<br><br>
    Learns a per-elephant identity embedding to personalise anomaly detection across individuals.<br><br>
    ...
    st.markdown("---")
    st.markdown("""
    <div class='section-label'>Elephants</div>
    """, unsafe_allow_html=True)
...
# ═══════════════════════════════════════════════════════════════════
# TAB 1 — Full 24-step window via generate / paste
# ═══════════════════════════════════════════════════════════════════
with tab1:
    ...
    if st.button("🔍 Run Detection", key="t1_run"):
        if model is None:
            ...
            if label == 0:
                ...
                st.markdown(f"""
                <div class='result-banner result-anomaly'>
...
# ═══════════════════════════════════════════════════════════════════
# TAB 2 — Quick manual single-point entry (repeats value across window)
# ═══════════════════════════════════════════════════════════════════
with tab2:
    st.markdown("""
    <div style='font-family:Space Mono,monospace;font-size:0.72rem;color:#6B8F74;margin-bottom:1.2rem;line-height:1.6'>
    Enter a representative GPS reading. The app replicates it across a 24-step window with small noise so you can quickly test different movement regimes.
    </div>
    """, unsafe_allow_html=True)
    ...
    if st.button("⚡ Quick Predict", key="q_run"):
        if model is None:
            ...
            # Speed gauge visual
            st.markdown("<div class='section-label' style='margin-top:1rem'>Speed Context</div>", unsafe_allow_html=True)
            gauge_pct = min(q_speed / 20.0, 1.0) * 100
            gauge_color = "#3DFF7A" if q_speed < 5 else ("#FFB830" if q_speed < 10 else "#FF4F6B")
            st.markdown(f"""
            <div style='margin-bottom:0.3rem;font-family:Space Mono,monospace;font-size:0.7rem;color:#6B8F74'>
              {q_speed:.1f} km/h  ·  Normal range: 0–5 km/h  ·  Anomalous: >10 km/h
            </div>
...
with tab3:
    ...
    steps = [
        ("01", "Data Collection", "Movebank GPS data for 4 Etosha elephants (LA11–LA14), year 2010. Features: lat, lon, timestamp."),
        ...
        ("03", "Window Creation", "24-timestep sliding windows. Label = 1 if >20% of steps in window are anomalous."),
        ("04", "Scaling", "StandardScaler fit on training set (LA11, LA12), applied to all splits."),
        ("05", "Model: LSTM + Embedding", "Elephant identity (LA11–LA14) is embedded as a 4-dim learnable vector, repeated across all timesteps and concatenated with movement features before LSTM processing."),
        ("06", "Inference", "New window → scale → [movement, id] → model → sigmoid probability → threshold 0.6 → label."),
    ]
    ...
    st.markdown("<div class='section-label' style='margin-top:1.5rem'>Model Architecture</div>", unsafe_allow_html=True)
    st.markdown("""
    <div style='font-family:Space Mono,monospace;font-size:0.72rem;color:#6B8F74;line-height:2;background:#111A15;border:1px solid #1E3024;border-radius:10px;padding:1rem 1.4rem'>
    Input: movement (24, 4)  +  elephant_id (1,)<br>
    ──► Embedding(4, 4) → Reshape(4,) → RepeatVector(24) → (24, 4)<br>
    ──► Concatenate → (24, 8)<br>
    ──► LSTM(64, dropout=0.3, recurrent_dropout=0.15) → (64,)<br>
    ──► Dense(32, relu)<br>
    ──► Dropout(0.3)<br>
    ──► Dense(1, sigmoid) → <span style='color:#3DFF7A'>anomaly probability</span>
    </div>""", unsafe_allow_html=True)

    st.markdown("<div class='section-label' style='margin-top:1.5rem'>What counts as anomalous?</div>", unsafe_allow_html=True)
    st.markdown("""
    <div style='font-family:Space Mono,monospace;font-size:0.72rem;color:#6B8F74;line-height:1.8'>
    An elephant movement window is flagged as anomalous when it contains an unusually high proportion of high-speed readings (above the 95th percentile of training speed).
    In practice this corresponds to speeds above roughly <span style='color:#FFB830'>~10 km/h</span> — indicating fleeing, charging, or other stress-related behaviour.
    Normal foraging and walking produces speeds of <span style='color:#3DFF7A'>1–5 km/h</span>.
    </div>""", unsafe_allow_html=True)
